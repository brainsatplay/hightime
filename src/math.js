 ...
 * We'll add more useful export function things like filter kernels etc. as we get to making them.
 * 
 ...
 */
	...
	export const TWO_PI = Math.PI*2; 			//2PI
	export const C = 299792458; 				//speed of light m/s
	export const G = 6.67430e-11; 			//Newton's gravitation constant N*m^2 / kg^2
	export const h = 6.62607015e-34; 			//Planck constant J*s
	export const R = 8.31432e3; 				//Universal gas constant J / kg*mol*K
	export const Ra = 287; 					//Air gas constant J / kg*K
	export const H = 69.3; 					//Hubble constant km/s/Mpc 
	export const kbar = 1.054571817e-34; 		//Dirac constant J*s
	export const kB = 1.380649e-23; 			//Boltzmann constant J/K
	export const ke = 8.9875517923e9; 		//Coulomb constant kg * m^3 * s^-2 * C^-2
	export const me = 9.1093837015e-31; 		//electron mass kg
	export const mp = 1.67262192369e-27; 		//proton mass kg
	export const mn =	1.67492749804e-27; 		//neutron mass kg
	export const P0 = 1.01325e5; 				//Sea level pressure N/m^2
	export const T0 = 288.15; 				//Sea level room temperature K
	export const p0 = 1.225; 					//Sea level air density kg/m^3
	export const Na = 6.0220978e23; 			//Avogadro's number 1 / kg*mol
	export const y = 1.405; 					//Adiabatic constant
	export const M0 = 28.96643; 				//Sea level molecular weight
	export const g0 = 9.80665; 				//Sea level gravity m/s^2
	export const Re = 6.3781e6; 				//Earth radius m
	export const B = 1.458e-6; 				//Thermal constant Kg / m*s*sqrt(kg)
	export const S = 110.4; 					//Sutherland's constant K
	export const Sigma = 3.65e-10; 			//Collision diameter of air m

	export const imgkernels = {
		edgeDetection: [
		  [-1, -1, -1],
		  [-1,  8, -1],
		  [-1, -1, -1]
	...
	export function genSineWave(freq=20,peakAmp=1,nSec=1,fs=512,freq2=0,peakAmp2=1){
		var sineWave = [];
		var t = [];
	...
	export function getSineAmplitude(frequency=20,peakAmplitude=1,ti=0, tOffset=0) {
	...
	//standard deviation
	export function std(arr,mean=undefined){
		let avg = mean; 
	...
	export function relError(actual=[],forecast=[],abs=true) {
	...
	export function informationEntropy(probabilities=[]) {
		let entropy = [];
	...
	export function variance(arr) { //Variance of 1D input arrays of length n
		var mean = this.mean(arr);
		return arr.reduce((a,b) => a + ((b - mean)**2), 0)/arr.length;
	}

	export function dot(vec1,vec2) { //nDimensional vector dot product
        var dot=0;
    ...
    export function cross3D(vec1,vec2) { //3D vector cross product
        return [
    ...
    export function magnitude(vec) { //nDimensional magnitude
        var sqrd = 0;
        vec.forEach((c) => {
            sqrd+=c*c;
        })
    ...
    export function distance(point1, point2) { //nDimensional vector distance function
        var dsqrd = 0;
        point1.forEach((c,i) => {
	...
	export function normalize(vec) { //nDimensional vector normalization
        var norm = 0;
	...
	export function newtonsMethod(foo=(x)=>{return Math.pow(x,5) + x*x - x - 0.2}, start=0,end=1, precision=0.01, attempts=10) {
		let roots = [];
    ...
    export const integral = (func=(x)=>{ let y=x; return y;}, range=[], stepx=0.01) => {
        let area = 0;
    ...
    export const dintegral = (func=(x,y)=>{ let z = x+y; return z;}, range=[[],[]], stepx=0.01,stepy=stepx) => {
        let volume = 0;
    ...
    export const tintegral = (func=(x,y,z)=>{ let w=x+y+z; return w;}, range=[[],[],[]], stepx=0.01, stepy=stepx, stepz=stepx) => {
        let volume = 0;
        for(let i = range[0][0]+stepx; i<range[0][1]; i+=stepx) {
            for(let j = range[1][0]+stepy; j<range[1][1]; j+=stepy) {
    ...
    export const pintegral = (func=(x)=>{ let y=x; return y; }, range=[], stepx=0.01) => {
        let length = 0;
        let y0 = undefined;
        let yi = undefined;
        for(let i = range[0]; i<range[1]; i+=stepx) {
    ...
    export function makeVec(point1,point2) {  //Make vector from two nDimensional points (arrays)
        var vec = [];
        point1.forEach((c,i) => {
            vec.push(point2[i]-c);
        })
	...
	export function matscale(mat,scalar) {
		let m = [];
	...
	export function histogram(arr=[], binSize=1, nBins=undefined) {  
		let copy = [...arr]; 
	...
	export function normalDistribution(samples=[], normalize=true, cutoff = 0.0001) {
	...
	export function expectedValue(samples=[],probabilities=this.normalDistribution(samples)) {
		return samples.reduce((sum,item,idx) => sum + item*probabilities[idx]);
	}

	//moment about the origin (statistics)
	export function originMoment(samples=[],probabilities=this.normalDistribution(samples),order=1) {
		return samples.reduce((sum,item,idx) => sum + Math.pow(item,order)*probabilities[idx]);
	}

	//moment about the population mean (statistics)
	export function centralMoment(samples=[],probabilities=this.normalDistribution(samples),order=1) {
		let m = this.mean(samples);
		return samples.reduce((sum,item,idx) => sum + Math.pow((item-m),order)*probabilities[idx]/samples.length);
	}

	export function linearDiscriminantAnalysis(samples=[], classifier=[]) {
	...
	export function conv1D(arr=[],kern=[1/3,1/3,1/3],pad=Math.floor(kern.length*0.5)) {
		let result = [];
	...
	export function conv2D(mat=[[],[],[]],kern=[[],[],[]],pad=0) {
	...
	export function cov2d(mat) { //[[x,y,z,w],[x,y,z,w],...] input list of vectors of the same length
	...
	export function cov1d(arr1=[],arr2=[]) {
		return this.cov2d([arr1,arr2]);
	}

	//3d covariance
	export function cov3d(x=[],y=[],z=[]) {
		return [
	...
	export function covNd(dimensionalData=[]) {
		let covariance = [];
	...
	export function eigens2x2(mat=[[1,2],[3,4]]) {
	...
	export function eigenvectors2x2(mat=[[1,2],[3,4]], eigens=[1,2]) {
	...
	export function fastpca2d(xarr,yarr){
	...
	export function crosscorrelation(arr1,arr2) {
	...
	export function autocorrelation(arr1) {
	...
	export function correlograms(dat=[[],[]]) {//Coherence network math for data pushed to the atlas
	...
	export function reduceArrByFactor(arr,factor=2) { //faster than interpolating
    ...
    export function makeArr(startValue, stopValue, nSteps) {
        var arr = [];
	...
	export function interpolateArray(data, fitCount, scalar=1) {
	...
	export function isExtrema(arr,critical='peak') { //Checks if the middle point of the (odd-numbered) array is a local extrema. options: 'peak','valley','tangent'. Even numbered arrays are popped
        let ref = [...arr];
    ...
    export function isCriticalPoint(arr,critical='peak') { //Checks if the middle point of the (odd-numbered) array is a critical point. options: 'peak','valley','tangent'. Even numbered arrays are popped
        let ref = [...arr];
		if(ref.length%2 === 0) ref.pop();
        if(arr.length > 1) { 
            let pass = true;
    ...
    export const peakDetect = (smoothedArray,type='peak',window=49) => {
    ...
    export function getPeakThreshold(arr,peakIndices, thresholdVar) {
        let threshold;
	...
	export function flatten_vector(v) {
		let v_new = [];
	...
	export function squared_difference(v1, v2) {
		let sum = 0.0;
	...
	export function shift_deflate(mat, eigenvalue, eigenvector)  {
	...
	export function eigenvalue_of_vector(mat, eigenvector) {
		// Xt * M * x
	...
	export function power_iteration(mat, tolerance=0.00001, max_iterations=1000) {
	...
	//Input square 2D matrix
	export function eigens(mat, tolerance=0.0001, max_iterations=1000) {
	...
	export function pca(mat,tolerance = 0.00001) {
		let dims = mat.length;
	...
	export function p300(event_timestamps=[],raw_signal=[],signal_timestamps=[], sps=256) {
